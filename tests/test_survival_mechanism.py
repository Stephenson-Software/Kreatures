...
class TestGracePeriodMechanics:
    ...
    def test_attack_avoidance_during_grace_period(self):
        ...
        with patch.object(game.environment, 'getEntities', return_value=[attacker]):
            with patch.object(game.environment, 'getRandomEntity', return_value=game.playerCreature):
                with patch.object(attacker, 'getNextAction', return_value='fight'):
                    ...
                    with patch('random.randint', return_value=50):  # 50 <= 85, so attack should be avoided
...
class TestPlayerInitialization:
    ...
    def test_player_starts_with_protection(self):
        ...
        protection_messages = [msg for msg in game.playerCreature.log if 'protection' in msg.lower()]
...
class TestIntegrationScenarios:
    ...
    def test_protection_survives_multiple_attacks(self):
        ...
        for enemy in enemies:
            with patch('random.randint', return_value=25):  # 25 damage becomes ~15 with reduction
                if player.health > 0:
                    damage = 25
